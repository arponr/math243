...
\caption{A comparison of average number of cooperations (red) and
  ...
  closely, making cooperation much more likely to fixate.}
...
seen in figure \ref{fig:really} below.

\begin{figure}[h]
\caption{$10^6$ rounds (first four) and $2 \times 10^6$ rounds (second
  four) with the parameters $b=2$, $c = d = 0.05$, $N = I = 100$,
  $\omega = 0.1$, $\mu = 0.0001$. Green trend lines are average
  reputation, blue are average strategy, and red are average
  fitness. Observe that in the first two runs, cooperation emerges
  even after a selfish strategy has fixated; in the fifth run,
  cooperation is invaded by a high-threshold, selfish strategy.}
...
We have developed a model for reputation dynamics that is robust
